...
export default class Lab4App extends cs380.BaseApp {
  async initialize() {
    ...
    this.cube = new cs380.PickableObject(cubeMesh, simpleShader, pickingShader, 2);
    ...
    vec3.set(this.cube.transform.localPosition, 1.5, 0, 0);
  ...
  onKeyDown(key) {
    ...
    let object = null;
    if (key === "s") object = this.sphere;
    else if (key === "c") object = this.cube;

    if (object) vec3.set(object.transform.localScale, 2*Math.random(), 2*Math.random(), 2*Math.random());
  }
  ...
  onMouseDown(e) {
    ...
    let object = null;
    if (index === this.sphere.pickingID) object = this.sphere;
    else if (index === this.cube.pickingID) object = this.cube;
    
    if (object) {
      quat.setAxisAngle(
        object.transform.localRotation,
        vec3.normalize(vec3.create(), vec3.fromValues(Math.random(), Math.random(), Math.random())),
        Math.PI * Math.random()
      )
    }
  }
